...
async fn handle_swarm_command(
    ...
    match command {
        SwarmCommand::DialPeer { peer_id, addrs } => {
            ...
            push_log(
                &mut status_guard,
                format!("Dial РЅР°С‡Р°С‚ РґР»СЏ peer {peer_id} via {} Р°РґСЂРµСЃ(Р°).", addrs.len()),
            );
        }
        SwarmCommand::KickPeer { peer_id } => {
            ...
            push_log(
                &mut status_guard,
                format!("Peer {peer_id} РѕС‚РєР»СЋС‡С‘РЅ via disconnect_peer_id()."),
            );
        }
    }
...
async fn handle_swarm_event(
    ...
    match event {
        ...
        SwarmEvent::ExternalAddrConfirmed { address } => {
            ...
            push_log(
                &mut status_guard,
                format!("Confirmed РІРЅРµС€РЅРёР№ swarm address: {address}"),
            );
        }
        SwarmEvent::ConnectionEstablished { peer_id, endpoint, .. } => {
            ...
            if relay_peer_ids.contains(&peer_id) {
                ...
                push_log(
                    &mut status_guard,
                    format!("РџРѕРґРєР»СЋС‡С‘РЅ relay bootstrap {peer_id} via {addr}"),
                );
                if relayed {
                    let _ = app.emit(
                        "relay_active",
                ...
                status_guard.note = Some(if config.mode == SessionMode::Client {
                    ...
                    "Peer РїРѕРґРєР»СЋС‡С‘РЅ Рє РІР°С€РµРјСѓ С…РѕСЃС‚Сѓ via libp2p stream.".into()
                });
                push_log(
                    &mut status_guard,
                    format!(
                        "Peer {peer_id} РїРѕРґРєР»СЋС‡С‘РЅ via {} ({addr})",
...
async fn ensure_relay_reservations(
    ...
    for relay_addr in relay_bootstraps {
        ...
        swarm
            ...
            .with_context(|| format!("РЅРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РїСЂРѕСЃРёС‚СЊ reservation via {relay_addr}"))?;
        ...
        push_log(
            &mut status_guard,
            format!("Requested Circuit Relay reservation via {relay_key}"),
        );
    }
